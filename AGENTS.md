...
| `tools/smoke-lab.cmd` | **MAP97 smoke lab** — unattended capture of muzzle smoke in a controlled room (`python tools/build_smoke_lab.py` first). `tools/smoke-sweep.cmd <cvar> <values...>` walks one cvar at a fixed tic for a frame-by-frame comparison. Use this rather than judging smoke in a real level. |
