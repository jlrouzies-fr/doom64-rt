...
# SMONF's blink, re-timed 25% faster (2026-08-22 request).
#
# Retribution's own ANIMDEFS run is 5,5,5,6,5,5 = 31 tics for the round trip
# SMONF1 -> F4 -> back. Scaling by 1/1.25 gives 4,4,4,5,4,4 = 25 tics, i.e.
# 31/25 = 1.24x -- 25% faster to within one tic, which is the finest GZDoom
# resolves (tics are integers; there is no 24.8).
#
# NOTE the chain does NOT include SMONF or SMONF5: the mod animates SMONF1 and
# uses SMONF and SMONF5 as separate static textures. MAP06 sector 142 is one of
# those statics, which is why whatsthat there reports a plain 'SMONF'.
#
# Shipped as our own ANIMDEFS lump rather than an edit of the mod's: GZDoom
# takes the LAST definition it loads for a texture, and our wad loads after the
# mod, so redefining the chain replaces it without touching Retribution's file.
# Same mechanism make_seqlight_fix.py's animdefs_lump() documents.
SMONF_ANIM = [("SMONF1", 4), ("SMONF2", 4), ("SMONF3", 4),
              ("SMONF4", 5), ("SMONF3", 4), ("SMONF2", 4)]
SMONF_ANIM_WAD = PROJ_ROOT / r"Doom64-Retribution\d64r-smonf-blink.wad"
...
def make_e_blue_indicator(
    ...
    *,
    gain: float = 1.25,
    green_scale: float = 0.45,
    blue_over: int = 28,
    blue_min: int = 70,
) -> Image.Image:
    """Bulb + LED row only: blue-dominant texels, the frame's own RGB kept.
    ...
    green_scale pulls the hue from CYAN to BLUE. The raw art is cyan -- green
    runs 60-82% of blue on the lit texels, and worst on the brightest frames,
    so the bulb read cyan exactly when it was most visible (reported
    2026-08-22). Scaling green rather than substituting a flat blue keeps the
    per-frame ramp and the bulb's own highlight shape intact; at 0.45 the lit
    texels land at G/B ~0.31, which reads blue.
    """
    ...
    for y in range(img.size[1]):
        for x in range(img.size[0]):
            ...
            op[x, y] = (
                min(255, int(r * gain)),
                min(255, int(g * gain * green_scale)),
                min(255, int(b * gain)),
                255,
            )
    return out
...
def write_animdefs_wad(path: Path) -> None:
    """A one-lump PWAD carrying only our re-timed SMONF blink."""
    body = "\n".join(
        [
            "// Generated by tools/fix_smon_emissives.py -- do not edit by hand.",
            "//",
            "// SMONF's blink, 25% faster than Retribution's own 5,5,5,6,5,5 (31 tics).",
            "// Loads after the mod, and GZDoom takes the LAST definition for a texture,",
            "// so this replaces the run without touching the mod's ANIMDEFS lump.",
            "",
            "texture SMONF1",
            "allowdecals",
        ]
        + [f"pic {n} tics {t}" for n, t in SMONF_ANIM]
        + [""]
    ).encode("ascii")
    ...
    for name, data in items:
        ...
        offset += len(data)
    path.write_bytes(
        struct.pack("<4sII", b"PWAD", len(items), 12 + len(blob)) + blob + directory
    )
...
def apply(raw: dict[str, bytes], z: zipfile.ZipFile) -> None:
    ...
    # --- the re-timed blink ------------------------------------------------
    write_animdefs_wad(SMONF_ANIM_WAD)
    old = 5 + 5 + 5 + 6 + 5 + 5
    new = sum(t for _, t in SMONF_ANIM)
    print(f"  {SMONF_ANIM_WAD.name}: blink {old} -> {new} tics "
