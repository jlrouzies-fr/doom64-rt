...
def main() -> int:
    ...
    for frame, floor in FLOORS.items():
        # Every rotation of the frame, at the frame's reviewed floor. None of the
        # attack frames share a lump with another frame (only the walk cycle does
        # -- VILEA1D1 and friends), so there is no threshold conflict to resolve.
        names = sorted(n for n in lumps
                       if len(n) > 4 and n[4] == frame
                       and not (len(n) > 6 and n[6] != frame))
        for tex in names:
            albedo = Image.open(io.BytesIO(z.read(lumps[tex]))).convert("RGBA")
            e_img, peak = build_e(albedo, floor)
            if peak == 0:
                # Deliberate for G and the heal frames. Still clear any stale
                # light keys so an earlier run cannot leave a chest light behind.
                entries[tex] = {}
                skipped += 1
                continue
            for d in MAT_DIRS:
            ...
            entries[tex] = {"emissiveMult": EMISSIVE_MULT}
            written += 1

    patch_global(entries)
    lit = {f for f, v in FLOORS.items() if v < 300}
    print(f"VILE glow: {written} frames masked, {skipped} deliberately empty")
    print(f"  lit frames   {''.join(sorted(lit))}")
    print(f"  empty frames {''.join(f for f in FLOORS if f not in lit)}  (wind-up + heal)")
