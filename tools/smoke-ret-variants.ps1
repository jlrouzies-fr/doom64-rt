...
'@
function Smoke($name, $arg) {
  Get-Process gzdoom -EA SilentlyContinue | Stop-Process -Force
  Start-Sleep -Milliseconds 300
  $p = Start-Process "$wd\gzdoom.exe" -WorkingDirectory $wd -ArgumentList $arg -PassThru
  Start-Sleep 1; [CL8]::Go([uint32]$p.Id)
  $parts = @()
  foreach ($sec in 3, 8, 15, 20) {
    ...
    try { $p.Refresh() } catch {}
    if ($p.HasExited) { $parts += "${sec}s=EXIT"; break } else { $parts += "${sec}s=$($p.Responding)" }
  }
