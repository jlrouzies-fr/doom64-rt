# Doom 64: Retribution — Path Tracing Implementation Plan (Windows)
...
**Reference projects:**
- [vs-shirokii/gzdoom-rt](https://github.com/vs-shirokii/gzdoom-rt) — path tracing renderer already built into a GZDoom fork. **This is our base engine.**
- [sultim-t/prboom-plus-rt](https://github.com/sultim-t/prboom-plus-rt) — same author's RTGL1 integration for PrBoom+, useful as a secondary reference for RTGL1 usage patterns.
- [postmemetic/Duke-RT](https://github.com/postmemetic/Duke-RT) — reference for material-authoring workflow (PBR overlays on classic-era textures) and NRI-based RT abstraction (not directly reusable since gzdoom-rt uses RTGL1, but useful for material pipeline ideas).
- [Doom 64: Retribution (ModDB)](https://www.moddb.com/mods/doom-64-retribution) — GZDoom total-conversion mod containing the actual Doom 64 content (maps in UDMF, monsters, weapons, sector lighting) that we will load into gzdoom-rt.
...
| **Current phase** | **Phase 3→4** — material spec done; MAP01 auto-PBR stub ready to playtest |
| **Blocked on** | User playtest of MAP01 PBR stubs (`go` / launch script) |
| **Next action** | Confirm metal/roughness/glow on MAP01; then expand Phase 4 beyond MAP01 |
...
| 3 Material / lighting authoring spec | **Done** — `material-authoring-spec.md` |
| 4 Author RT content for Retribution | **Started** — MAP01 auto stubs via `tools/gen_map01_pbr.py` |
| 5 Integration / load order / launcher | Not started |
| 6 Performance tuning | Not started |
...
| `G:\AI\Doom64-RT\sourcecode\gzdoom-rt\` | **Primary engine** — patched fork; build → `build/RelWithDebInfo/` |
...
- As of v1.5 it is **not an IWAD**. Load: `-iwad DOOM2.WAD -file D64RTR[…].WAD [brightmaps]`.
- **Stock gzdoom-rt is not mod-friendly:** Steam DOOM2 size/launcher gates + `rt/scenes/map##` keyed only by map name → Retribution MAP01 pulls Doom II light scenes (crash/corruption risk). **Source fork required** (Phase 2.5).
- Inventory: **35 UDMF maps**, **DECORATE only** (no ZSCRIPT), `GLDEFS`/`ANIMDEFS`/`TEXTURES`, **1187 TX_** textures, 1387 sprites.
- PowerShell: `[v1.5]` in paths is a wildcard — use `D64RTR_v15.WAD` copy or `-LiteralPath`.
...
Until rebuild finishes, stock prebuilt can limp with Steam-sized IWAD + `-rtdoom2` + (workaround) moving `rt\scenes\map*` aside — not a real fix.

---

## 0. Prerequisites & Environment Setup

- [x] Windows 10/11, NVIDIA RTX GPU (20-series or newer) with current Game Ready / Studio drivers — **RTX 5090 confirmed**
- [ ] Visual Studio 2022 (Desktop C++ workload) — **deferred** (prebuilt used; `cl` not on PATH yet)
- [x] CMake ≥ 3.20 — present
- [x] Git (with submodule support) — present
- [ ] Vulkan SDK (latest) — **deferred** (`VULKAN_SDK` not set; only needed for source builds)
- [x] ~20 GB free disk space (engine build + WAD assets + intermediate shader caches) — assumed OK given downloads present
...
- [x] **Validation:** Run with vanilla DOOM2.WAD — **user confirmed Doom II RT works** (2026-08-02).

**Cursor task:** Automate into `setup.ps1` — optional; skipped while prebuilt path is sufficient.

---

## 1. Acquire and Inspect Doom 64: Retribution

- [x] Download Doom 64: Retribution from ModDB — present at `Doom64-Retribution/`
- [x] Confirm package layout — main content is **`D64RTR[v1.5].WAD`** (PWAD); brightmaps in `D64RTR_BRIGHTMAPS.PK3`
- [x] Inventory (scripted; SLADE optional for deep dives):
  - Map format: **UDMF** (`TEXTMAP` + `BEHAVIOR` + `ZNODES`) for MAP00–MAP34 (35 maps)
...
**Cursor task:** [x] Inventory script → `tools/inventory_retribution.py` → `retribution-asset-inventory.md`
...
- [ ] Check `sourcecode/gzdoom-rt/docs/` and Duke-RT `MATERIAL-OVERLAY-AUTHORING.md` / `LIGHTOVR-AUTHORING.md` for workflow ideas (different backend — adapt carefully)
...
Bulk creative/technical work — Doom 64 look into RTGL1 materials. Use inventory TX_ names (lava, nukage, water, switches, etc.) and brightmaps as emissive hints.
...
- [ ] Package Phase 4 work as overlay pk3 (e.g. `Retribution-RT-Materials.pk3`) — never edit Retribution in place
- [ ] Load order: `DOOM2.WAD` → `D64RTR[v1.5].WAD` → `D64RTR_BRIGHTMAPS.PK3` (optional) → `Retribution-RT-Materials.pk3` last
...
|---|---|---|
| gzdoom-rt GZDoom version behind Retribution → script errors | Medium | Phase 2 catch; patch DECORATE/MAPINFO; log in `compat-patches.md` |
...
1. ~~**Session 1:** Phase 0 — validate vanilla gzdoom-rt~~ **Done**
2. ~~**Session 2:** Phase 1 — inventory Retribution~~ **Done**
3. **Session 3–4 (current):** Phase 2 — Retribution in gzdoom-rt, compat fixes
4. **Session 5:** Phase 3 — material-authoring spec
5. **Session 6–9:** Phase 4 — emissive/material data
6. **Session 10:** Phase 5 — overlay pk3, launcher
7. **Session 11:** Phase 6 — performance
8. **Session 12:** Phase 7–8 — polish and packaging
