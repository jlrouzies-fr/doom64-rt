...
### 1.6h Fence casts no shadow under a lamp ceiling — **ENGINE FIX 2026-08-14** (needs confirm)
...
| **Already eliminated, do not re-test** | The fence is in the BLAS (`PT_ALPHA_TESTED`, `FORCE_NO_OPAQUE`, `WORLD_0`, vertex alpha forced to 1 by `rt_force_mask_opaque`) — commit `aacb140`. Shadow rays run `RtAlphaTest.rahit`, so the holes pass light (`RaygenCommon.h:338-361` traces with `SkipClosestHit` only, never `RayFlagsOpaque`). No fence texture carries `noShadow`. And `rt_debug_visibility 1` showed visibility returning **1.0** — the ray is genuinely unblocked, not blocked-then-washed (`f7cee45`). |
| **Cause A — source radius** | `rt_ceiling_edge_radius` / `rt_wall_strip_radius` were **0.35 m = 11.2 map units**, against 0.02 = 0.64 for the flashlight and muzzle flash. Penumbra scales with source size, so an 11-unit source erases a 4-unit wire's shadow and leaves wide occluders alone — exactly the observed split. Diagnosed 2026-08-08 (`60e19bb`, `aacb140`, `df13cd9`); **the fix was never shipped**, and the finding was never written into `docs/`. |
| **Cause B — array extent** | Since the bulb lattice (§1.6g, 2026-08-10) a pane is lit by ~100 lights at `rt_ceiling_bulb_spacing` 16 across the whole ceiling. That array **is** an area source hundreds of units wide regardless of each member's radius, so Cause A's fix alone cannot reach it. Not visible in August because the lattice did not exist yet. |
| **Why the old ladders can't find B** | `ab-bulb-density`, `ab-bulb-keyfill`, `ab-lamp-placement` and `ab-bulb-softness`'s `pin` arm all thin the count with `rt_ceiling_edge_seglen`, which the lattice ignores. Their nulls are void, not negative. All four re-pointed at `rt_ceiling_bulb_spacing` on 2026-08-14. See `rt-lighting-practices.md` §34b. |
| **Fix** | Radii 0.35 → **0.15**; `rt_shadow_samples` **1 → 4**, pinned (at 1 a soft shadow is one binary sample and the denoiser averages it away). Plus `rt_ceiling_bulb_key` — a KEY/FILL split inside the pane: the dense lattice keeps `1 - share` of the flux and the even wash, a sparse set of compact lights (`_key_spacing` 128, `_key_radius` 0.04) on the same painted bulbs takes `share` and does the casting. Flux conserved by construction, so brightness must not move. `rt_lights_fixtures.cpp`, the `bulbLatticeFor` branch. |
| **Radius costs no brightness** | RTGL1 sets radiance = `intensity/(π r²)` (`LightManager.cpp:143-181`) and the sphere subtends `≈ π r²/d²` (`Light.h:168-174`), so the product is `intensity/d²`. Radius is a pure softness knob. If a room dims when you lower one, something else moved with it. |
| **A/B** | `.\tools\ab-bulb-key.cmd <off\|half\|strong\|keyonly> [map]`, default MAP01 at the cage. Judge in order: (1) readable **grey** wire shadow; (2) has the lamp band scalloped into beads; (3) brightness must **not** move between arms — if it does the compensation is wrong and every comparison is void. The console line reports `(K keyed, share S)`; **K = 0 on a non-`off` arm means the arm never reached the renderer**. |
| **Confirm** | Also `rt_debug_visibility 2` at the same spot — wire-shaped red on the floor is the unambiguous umbra, independent of denoising. Regression: MAP94's four `SFLATAS` panes must still measure floor ≈123; check MAP02's bloom room and MAP03's 46 thin recessed strips, the two rooms past changes here have broken. |
