...
Three features share that pk3: splats that **persist**, a splash on **explosive
kills**, and **per-monster blood colour**. This file is the live reference for
all three. `docs/blood-explosion-burst.md` is the design record for the second
...
These nine are declared in the **pk3's `CVARINFO`**, not in `rt_cvars.inc` —
the feature is a pk3, so its cvars are too. All `noarchive`, all pinned in
`tools/d64rt-pins.cfg` (the launcher `+exec`s it; the pins moved out of
`launch-retribution-rt.cmd` when its command line hit cmd.exe's 8191-char
limit) and restated in every `ab-blood.cmd` arm.

`tools/check_pins.py` reads pk3 `CVARINFO` lumps as well as the engine's X-macro
list, so these do not show up as orphaned pins. A tenth cvar,
`rt_tex_translations`, belongs to the engine and is covered under the colour
section below.
...
| `rt_gore_burst_debug` | `false` | one `Console.Printf` per burst — the is-it-live instrument. Prints `RTBloodBurst: <class> dmg N -> M splats  bloodcolor r,g,b`. |
...
- `P_RadiusAttack` (`p_map.cpp:6136`) calls `P_DamageMobj(..., DMG_EXPLOSION)`
...
no blood is the **negative control**, not a failure. `noboom` is the flip and
keeps the debug print on deliberately — it should produce **no** lines, so a
line appearing there means the arm did not take. `bigboom` brackets count/speed
from above. A splat can land on a ledge or stick
